...
export const getUserData = createAsyncThunk("chat/getUserData", async () => {
  ...
  try {
    const response = await axios.get("https://51.21.127.157:4000/api/user", {
      headers: {
  ...
  async () => {
    ...
    try {
      const response = await axios.get(
        "https://51.21.127.157:4000/api/project/currentProjects",
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Bearer " + token,
...
export const deleteChat = createAsyncThunk(
  ...
  async (chatId: string) => {
    const token = localStorage.getItem("token")!;
    try {
      await axios.delete(`https://51.21.127.157:4000/api/chat/${chatId}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: "Bearer " + token,
        },
